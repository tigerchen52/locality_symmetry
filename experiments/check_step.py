...
def main(
         ...
         data_file='data/evaluation2.txt',
         model_file=None,
         data_parallel=True,
    ...
    avg_loss = trainer.eval_loss(get_loss, model_file, None, data_parallel)
    return avg_loss


def multi_epoch():
    # loss = main(model_file=model_file)
    # print('loss = {a}'.format(a=loss))
    loss_list = list()
    for i in range(300, 29400, 600):
        mfile = 'model/pretrain/model_steps_' + str(i) + '.pt'
        print('model file = {a}'.format(a=mfile))
        loss = main(model_file=mfile)
        print('loss = {a}'.format(a=loss))
        loss_list.append(loss)
    if not os.path.exists('data/eval_loss'):
        os.mkdir('data/eval_loss')
    with open('data/eval_loss/eval_loss.txt', 'w', encoding='utf8')as f:
        f.write('\n'.join([str(round(loss, 2)) for loss in loss_list]))
